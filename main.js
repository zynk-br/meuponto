...
function createWindow() {
  ...
  mainWindow.webContents.on("did-finish-load", async () => {
    const nodeExists = await DependencyManager.checkNodeJs();
    if (!nodeExists) {
    ...
    if (browserExists) {
      ...
      const result = await DependencyManager.installBrowser(
        getLocalBrowserPath(),
        logToUI
      );
...
async function syncWithWebsite() {
  ...
  try {
    ...
    logToUI(
      `[INFO] Pontos encontrados para HOJE (${todayDateString}): ${
        sortedPunches.length > 0 ? sortedPunches.join(", ") : "Nenhum"
      }`
    );
...
async function monitorAndExecutePunches() {
  ...
  if (new Date() >= parseTime(nextPunch.time)) {
    ...
    try {
      ...
      const notificationMessage = `✅ Ponto de *${
        nextPunch.id
